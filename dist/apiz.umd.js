(function (global, factory) {
	...
	(global = global || self, factory(global.apizng = {}));
	...
	let defaultType, globalQuerystring, globalParamRegex, 
	// 这东西有没有, 是什么类型, 应该只能在运行时才能确定了, 或者分析控制流?
	// 那就随便写个类型吧...等到使用处as一下好了
	globalClient, globalImmutableMeta = false;
	...
	function parseApiInfo(name, rawInfo, { baseURL: gBaseURL, paramRegex, querystring, client }) {
	    ...
	    const info = {}, bURL = baseURL || gBaseURL;
	...
	function APIz(apiMeta, options) {
	    let baseURL, immutableMeta, paramRegex, querystring, client, meta = {};
	...
	function config({ querystring, paramRegex, immutableMeta, client, reset, defaultType: dt } = { reset: true }) {

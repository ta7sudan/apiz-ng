...
### `APIz(meta: APIMeta, options: APIzOptions)`

返回一个 APIz 实例 `apis`. `apis` 具有以下方法.

* `add(key: string, info: APIMetaInfo)`, 添加一个新的 API. eg.
...
* `interface APIzRequest`, HTTP 方法对应的请求方法, 如 `get()`, `post()` 等, 它的签名是
  ...
  ```typescript
  interface APIzRequest<RawRequestOptions, APIzClientType extends string = string, APIzClientMeta = any> {
  ...
  是根据 `APIMeta` 中的配置实例化的方法, 注意到它有多个重载

### `(body?: any, params?: KVObject, query?: KVObject | string, type?: APIzClientType): Promise<any>` 和 `(body: any, params: KVObject | string, type?: APIzClientType): Promise<any>` 和 `(body: any, type: APIzClientType): Promise<any>`

对于带 body 的请求, `body` 是必需的, 如果不需要, 可以传 `null`. 其他参数都是可选的, `params` 作为路径参数, 是一个 key - value 的对象, `query` 作为查询字符串, 是一个 key - value 对象或字符串, `type` 用来指定 body 的类型. 意味着你可以以下面这几种形式调用, 而不需要传递多余的参数.
...
`body` 可以是任意类型, 支持的类型取决于 `APIzClient` 的实现, `type: APIzClientType` 也是由 `APIzClient` 提供, 默认是一个字符串. 这样你可以传入类型为 `Buffer` 或 `ArrayBuffer` 的 `body` 也不会有什么问题, `type` 用来提示 `APIzClient` 应当如何序列化 `body` 或正确地设置 `Content-Type`.

如果配置了 `pathParams: true`, 则 `params` 会被解释成路径参数, 一旦设置了 `pathParams: true`, 则 `params` 是必需的.

`query` 可以是对象也可以是字符串, eg. `"key0=000&key1=111"`.

函数返回一个 `Promise`. 只有 `POST`, `PUT`, `PATCH` 的方法会对应以上几个方法签名, 尽管浏览器上支持 `DELETE` 和 `OPTIONS` 请求携带 body, 但是考虑到这样的场景比较少, 避免在大多数情况下不需要 body 的时候必须手动传入一个无意义的 `null`, 所以还是将 `DELETE` 和 `OPTIONS` 设计成不带 body 的. 另一方面是可能有一些服务端不支持 `DELETE` 携带 body, 参考 https://stackoverflow.com/questions/299628/is-an-entity-body-allowed-for-an-http-delete-request.

也许以后会考虑提供配置选项来指定哪些方法会被实例化成允许带 body 的.



### `(params: KVObject, query?: KVObject | string): Promise<any>` 和 `(query: KVObject | string): Promise<any>`

和前面几个签名类似, 但是作为不需要携带 body 的请求, 这类方法不需要 `body` 和 `type` 参数, 所以只会有以下几种调用形式.
...
函数返回一个 `Promise`. 只有 `GET`, `HEAD`, `DELETE`, `OPTIONS` 的 API 会被实例化成此类签名的方法. 尽管 HTTP 规范并没有限制 `GET` `DELETE` `OPTIONS` 是否可以带 body, 但是还是根据大多数场景以及大多数人的习惯, APIz 将它们设计成不带 body 的请求.
...
`APIMeta` 是一个 Object, 用来配置一组 API, API 可以是任意名字, 除了 `_baseURL`, 配置的名字会被作为 APIz 实例上的一个方法, 如前面的 `apis.getBook()`.

`_baseURL` 被作为 `APIzMeta` 属性中唯一一个保留名字, 用来配置该组 API 的 `hostname`, `port` 等, 有利于减少重复内容. 不过它不是必需的, 我们还可以通过 `APIzOptions` 在实例化的时候配置该组 API 的 `baseURL`.

`APIMeta` 中的每个属性都是一个 `APIMetaInfo` 对象.



### `APIMetaInfo`

`APIMetaInfo` 是一个 Object, 用来描述一个 API 的必要信息. 支持以下字段.

* `url`, string, 一个 API 的完整 URL, 一旦设置了它, 会忽略下面的 `baseURL` 和 `path` 以及该组的 `_baseURL` 和 `APIzOptions` 中的 `baseURL`
* `baseURL`, string, 它的优先级大于该组配置的 `_baseURL` 和 `APIzOptions` 中的 `baseURL`
...
* `pathParams`, boolean, 指示是否使用路径参数, 一旦指定则 `APIzRequest` 对应的方法必须指定 `params`
...
* `baseURL`, 同 `APIMeta` 中的 `_baseURL`, 不过它的优先级高一些, 优先级是 `APIMetaInfo.baseURL` > `APIzOptions.baseURL` > `APIMeta._baseURL`
...
* `defaultType`, 为所有 `APIMetaInfo` 指定默认的 `type`, 这样的话在配置 `APIMetaInfo` 的时候也可以省略 `type`, 注意它和 `APIMetaInfo` 中 `type` 的区别. 指定 `defaultType` 是可以在配置 `APIMetaInfo` 的时候省略 `type`, 指定 `APIMetaInfo` 的 `type` 是在调用带 body 的请求方法的时候可以省略 `type`
